...
`backend/rag/summary_agent.py` s’exécute après une synchronisation Chroma réussie. Son graphe `select → draft_sections → compose → save` reprend au plus `summary.top_n` P1 apparus depuis la modification du dernier rapport, triés par date de publication décroissante (date de collecte en repli). Un retrieval global enrichit l’ensemble, puis un seul appel Nyx rédige le rapport complet. Chaque génération est archivée en heure `Europe/Paris` sous `data/reports/report_YYMMDD_HHMM.md`, avec la date locale dans le titre ; `data/summary.md` reste la copie courante compatible. Homepage et Telegram chargent le dernier nom daté et se rabattent sur `summary.md` avant initialisation de l’archive. `/api/summary` répond avec `Cache-Control: no-store` et le nom du fichier effectivement servi. L’écriture est atomique. En cas d’échec, le fichier précédent et la fenêtre de reprise restent inchangés.

Le timer Atlas lance la pipeline `fetch → SQLite/scoring → embedding → synthèse → summarizer → Telegram` à 10 h, 14 h et 18 h. Le summarizer transforme le rapport complet en `telegram_YYMMDD_HHMM.txt`, sans Markdown ni références, avec le titre `Rapport DD-MM HH:MM` et des paragraphes aérés. `summarizer.max_output_tokens` dans `ai.yaml` borne directement la génération Ollama via `num_predict`; `summarizer.reasoning=false` réserve ce budget au contenu final plutôt qu’au raisonnement Qwen. La limite en caractères est ensuite validée avant l’envoi unique. Un dépassement ou une réponse vide déclenche une nouvelle tentative, jamais une coupe du texte. Le texte persistant est réutilisé après une panne Telegram. Une panne Nyx ne perd pas les articles : `rag_index_state.pending` reste vrai dans SQLite et la prochaine collecte reprend l’indexation. Une empreinte empêche les doublons. L’état et la dernière erreur sont visibles dans **Santé** et via `GET /api/rag/index/status`.
